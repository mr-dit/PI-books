<template>
    <div class="p-6 rounded-lg w-full text-white mx-auto mt-4">
        <h3 class="text-xl font-bold mb-2">Авторы книги: {{ bookTitle }}</h3>

        <!-- Выпадающий список для выбора автора -->
        <select v-if="authorList.length > 1" v-model="localSelectedAuthor" @change="updateSelectedAuthor" class="mb-4 bg-primary-500 border border-gray-300 p-2 rounded w-full text-white">
            <option v-for="auth in authorList" :key="auth.name" :value="auth">
                {{ auth.name }}
            </option>
        </select>

        <p class="text-sm text-white-700">{{ localSelectedAuthor.birthDate }} - {{ localSelectedAuthor.deathDate || 'Наше время' }}</p>
        ...
        <a v-if="localSelectedAuthor.wikipediaLink" :href="localSelectedAuthor.wikipediaLink" target="_blank" class="text-blue-500 underline mt-4 block">
            Читать подробнее
        </a>
    </div>
</template>
